...
export function renderNetworkSvg(
  ...
  books.forEach((book, index) => {
    const angle =
      (Math.PI * 2 * index) / Math.max(books.length, 1) - Math.PI / 2;
...
function renderRelationLines(
  ...
  [
    ...model.prerequisiteEdges,
    ...model.coStudyEdges,
    ...model.referenceEdges,
  ].forEach((relation) => {
...
function renderCoStudyHubs(
  ...
  model.coStudyGroups.forEach((group, index, groups) => {
    const angle =
      (Math.PI * 2 * index) / Math.max(groups.length, 1) - Math.PI / 2;
    const hub = {
