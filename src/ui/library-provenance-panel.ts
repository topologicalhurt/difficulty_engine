...
function provenanceLine(label: string, provenance?: EnrichmentFieldProvenance): HTMLElement | null {
  if (!provenance) return null;
  const confidence = formatPercent(provenance.confidence);
  const strategy = provenance.strategy ? ` · ${provenance.strategy.replace(/_/g, ' ')}` : '';
...
export function renderEnrichmentProvenanceCard(
  ...
  return card(
    ...
    el(
      ...
      el('div', { className: 'muted-copy', text: `Status: ${enrichmentStatus}` }),
      el('div', { className: 'muted-copy', text: enrichmentError ?? 'No enrichment errors.' }),
      ...
      book.openLibraryEditionKey
        ? el('div', { className: 'muted-copy', text: `Open Library edition: ${book.openLibraryEditionKey}` })
        : null,
      book.openLibraryWorkKey
        ? el('div', { className: 'muted-copy', text: `Open Library work: ${book.openLibraryWorkKey}` })
        : null,
      book.googleBooksId
        ? el('div', { className: 'muted-copy', text: `Google Books: ${book.googleBooksId}` })
        : null,
    ),
  );
}
