...
import { buildWorkloadProfiles, type WorkloadProfile } from './workload-profiles';
import {
...
function clusterTopPhrases(ids: string[], topicIndex: TopicIndex): string[] {
  ...
  ids.forEach((id) => {
    Object.entries(topicIndex.bookStats[id]?.topicWeights || {}).forEach(([topic, score]) => {
...
function clusterConfidence(
  ...
  const memberConfidence = mean(members.map((profile) => profile.metadataConfidence));
  const connectivity = mean(
    members.flatMap((profile) =>
      ids.filter((id) => id !== profile.id).map((id) => similarity[profile.id]?.[id] || 0),
    ),
  );
  return round2(
    clamp(
...
function similarityToCluster(
  ...
  return round2(
    mean(ids.filter((id) => id !== profile.id).map((id) => similarity[profile.id]?.[id] || 0)),
  );
}
...
function workloadPrior(
  ...
  const singletonSpecialized = members.length === 1 && Boolean(members[0]?.sparseSpecialized);
  return round1(
    singletonSpecialized
      ? Math.max(rawPrior, members[0]?.corpusComplexity || rawPrior, libraryMedianWorkload)
...
function buildAssignment(
  ...
  return {
    ...
    shrinkageApplied:
      ...
      profile.metadataConfidence < CLUSTER_CONFIDENCE_MODEL.shrinkageMetadataCutoff,
...
export function buildWorkloadClusters(
  ...
  const byProfile = Object.fromEntries(profiles.map((profile) => [profile.id, profile]));
  const libraryMedianWorkload = round1(median(profiles.map((profile) => profile.initialWorkload)));
  const similarity = buildWorkloadSimilarityMatrix(profiles, relationInfo);
  const byBookId: Record<string, WorkloadBookAssignment> = {};
  const clusters: WorkloadCluster[] = connectedWorkloadComponents(profiles, similarity).map(
    (ids, index) => {
      ...
      const prior = workloadPrior(members, rawPrior, confidence, libraryMedianWorkload);
      const clusterId = `workload-${index + 1}`;
      const assignments = members.map((profile) => {
        const assignment = buildAssignment(profile, clusterId, ids, prior, confidence, similarity);
        byBookId[profile.id] = assignment;
        return assignment;
      });
      return {
        id: clusterId,
        label: topPhrases.slice(0, 2).join(' / ') || `Workload cluster ${index + 1}`,
        bookIds: ids,
        topPhrases,
        workloadPrior: prior,
        confidence,
        evidenceSources: unique(members.flatMap((profile) => profile.evidenceSources)),
        assignments,
      };
    },
  );
