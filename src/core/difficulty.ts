...
import { difficultyDistributionStats, mapDisplayDifficulty } from './difficulty-mapping';
import type { CorpusSnapshot, DifficultyModelSnapshot, RelationInfo, TopicIndex, WorkloadClusterSnapshot } from './internal-types';
...
function effectiveSeed(book: { lockDiff: boolean; manualSeedDifficulty: number; seedEstimate: number }): number {
  const manualSeed = clamp(safeNumber(book.manualSeedDifficulty, NEUTRAL_MANUAL_SEED), 1, 10);
  ...
  return clamp(
    manualSeed * MANUAL_SEED_BLEND_WEIGHT + corpusSeed * CORPUS_SEED_BLEND_WEIGHT,
    1,
    10,
  );
}
...
function workloadStrength(project: PlannerProjectV1): number {
  return clamp(safeNumber(project.constraints.subjectWorkloadStrength, SUBJECT_WORKLOAD_DEFAULT), 0, 100) / 100;
}
...
function subjectWorkloadLift(
  ...
  return round2(
    clamp(
      (clusterPrior - baseDifficulty) * strength * clamp(clusterConfidence, 0, 1),
      -WORKLOAD_LIFT_CAP,
      WORKLOAD_LIFT_CAP,
    ),
  );
...
export function computeDifficultyModel(
  ...
  const order = [...ids].sort((left, right) => (depths[left] - depths[right]) || left.localeCompare(right));
  ...
  order.forEach((id) => {
    ...
    const prereqs = (relationInfo.prereqById[id] || []).filter((parent) => model[parent] && !corpus.byId[parent]?.noPropOut);
    const parentScores = prereqs.map((parent) => model[parent]?.scheduleDifficulty || 0);
    const graphBurden = round2(
      prereqs.length
        ? (mean(parentScores) - seed) * GRAPH_BURDEN_PARENT_WEIGHT + (depths[id] || 0) * GRAPH_BURDEN_DEPTH_WEIGHT
        : 0,
    );
    ...
    const retention = round2(clamp((mean(parentScores) * safeNumber(project.constraints.prereqRetention, 0.45)) / 10, 0, 1.5));
    const noveltyLoad = round2(novelty * safeNumber(project.constraints.propNovelty, 0.18) * NOVELTY_LOAD_MULTIPLIER);
    const breadthLoad = round2(breadth * safeNumber(project.constraints.propBreadth, 0.12) * BREADTH_LOAD_MULTIPLIER);
    ...
    const subjectWorkloadPrior = workload?.subjectWorkloadPrior ?? round1(baseDifficulty);
    ...
    const graphCap = Math.max(
      ...
      Math.abs(workloadBaseDifficulty) * clamp(safeNumber(project.constraints.alphaCap, 0.5), 0, 1),
    );
    const absoluteLiftCap = Math.max(
      0,
      safeNumber(project.constraints.propLiftCap, graphCap),
    );
    const cappedLift = clamp(graphLift, -Math.min(graphCap, absoluteLiftCap), Math.min(graphCap, absoluteLiftCap));
    const propagatedDifficulty = clamp(workloadBaseDifficulty + cappedLift, 1, 10);
    const blendedDifficulty =
      project.constraints.blendMode === 'linear'
        ? propagatedDifficulty
        : Math.sqrt(Math.max(0.1, workloadBaseDifficulty) * Math.max(0.1, propagatedDifficulty));
    const scheduleDifficulty = round1(
      clamp(blendedDifficulty, 1, 10),
    );
    ...
    model[id] = {
      ...
      explanation: [
        ...
        prereqs.length ? `${prereqs.length} prerequisite(s) contribute graph burden ${round2(graphBurden)}.` : 'No prerequisite burden contributes to this score.',
