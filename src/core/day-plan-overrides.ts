...
import type { CalendarActualOverride, CalendarEntry, PlannerProjectV1 } from './types';
...
export function tenthsForActualMinutes(state: PlanningState, actualMinutes: number): number {
...
export function tenthsForActualPages(state: PlanningState, actualPages: number): number {
  const pages = Math.max(0, safeNumber(actualPages, 0));
  return Math.min(state.remainingTenths, Math.round(pages * 10));
}

export function hasActualProgressOverride(override: CalendarActualOverride | null): boolean {
...
export function tenthsForActualOverride(
  ...
  if (override?.pages != null) return tenthsForActualPages(state, override.pages);
  if (override?.minutes != null) return tenthsForActualMinutes(state, override.minutes);
