...
export function choosePlannedDaysForState(
  ...
  const strictMinTenths = Math.max(10, Math.round((state.strictMinPg || bounds.minPg) * 10));
  let minTenths = Math.max(
    10,
    Math.round((state.effectiveMinPg || state.strictMinPg || bounds.minPg) * 10),
  );
    ...
    state.totalTenths
      ? state.mppRead *
          ((state.readTotalTenths / state.totalTenths) +
            (state.skimTotalTenths / state.totalTenths) * (state.skimRatio || 0.35))
      : state.mppRead,
  );
  const maxTenthsByTime = Math.max(
    1,
    Math.floor((totalBudgetMinutes(project.constraints) / averageMpp) * 10 + TIME_FIT_EPSILON),
  );
  ...
  const lowerByTime = Math.ceil(state.totalTenths / Math.max(1, maxTenthsFeasible));
  const upperByPages =
    state.totalTenths < minTenths ? 1 : Math.floor(state.totalTenths / Math.max(1, minTenths)) || 1;
  ...
  if (isPracticalMode && maxTenthsByTime >= 10 && upper < lower) {
    minTenths = Math.max(10, Math.min(minTenths, Math.floor(state.totalTenths / lower)));
    state.effectiveMinPg = minTenths / 10;
    state.floorRelaxed = state.effectiveMinPg < state.strictMinPg - FLOOR_RELAXATION_EPSILON;
  ...
  if (state.manualDaysLocked) {
    ...
    state.targetDayPages = round1((state.totalTenths / 10) / Math.max(1, state.planDays));
