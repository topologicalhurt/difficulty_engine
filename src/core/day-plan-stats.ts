...
export function buildDayPlanBookStats(
  ...
  states.forEach((state) => {
    ...
    byBookStats[state.id] = {
      ...
      desiredPagesPerDay: state.desiredPagesPerDay,
      feasibleMinPagesPerDay: state.feasibleMinPagesPerDay,
      feasibleMaxPagesPerDay: state.feasibleMaxPagesPerDay,
      finalPagesPerDay: state.finalPagesPerDay,
      pacingBindingReason: state.pacingBindingReason,
      peakDayPages: state.peakTenths / 10,
      boostedDays: state.boostedDays,
      unfinishedPages: state.unfinishedTenths / 10,
      readingRampFactor: state.readingRampFactor,
      readingRampStage: state.readingRampStage,
      readingRampReason: state.readingRampReason,
