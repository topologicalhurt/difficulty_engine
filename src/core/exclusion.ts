...
import { normalizeAutoResearchChainLength } from './constraint-normalizers';
import type {
  CorpusSnapshot,
...
export function computeExclusionState(
  ...
  const ignoredSet = new Set(
    corpus.books.filter((book) => book.ignored).map((book) => book.id),
  );
  const manualRDSet = new Set(
    corpus.books.filter((book) => book.constantRD).map((book) => book.id),
  );
  ...
  if (project.constraints.autoRD) {
    const activeIds = corpus.books
      ...
      .filter(
        (id) =>
          !ignoredSet.has(id) &&
          !manualRDSet.has(id) &&
          !(project.constraints.excComp && corpus.byId[id]?.completed),
      );
    ...
    const minLength = normalizeAutoResearchChainLength(
      project.constraints.rdMinChain,
    );
    ...
    activeIds.forEach((id) => {
      ...
      const difficulties = chain.map(
        (entry) =>
          difficultyModel.byId[entry]?.scheduleDifficulty ||
          corpus.byId[entry]?.manualSeedDifficulty ||
          5,
      );
      const deltas = difficulties
        .slice(1)
        .map((value, index) => value - difficulties[index]);
      const avgDelta = mean(deltas);
      const lowBranch = chain
        .slice(1, -1)
        .every(
          (entry) =>
            (incoming[entry] || []).length <= 1 &&
            (outgoing[entry] || []).length <= 1,
        );
      ...
      if (
        lowBranch &&
        avgDelta >= minSlope &&
        avgConfidence < AUTO_RD_CONFIDENCE_THRESHOLD
      ) {
        chains.push({
          ids: chain,
          ...
          label: chain
            .map((entry) => corpus.byId[entry]?.short || entry)
            .join(' -> '),
        });
