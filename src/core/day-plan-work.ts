...
export function splitTenths(
  ...
  const remaining = Math.max(0, Math.min(state.remainingTenths, Math.round(take || 0)));
  if (!remaining) return { readTenths: 0, skimTenths: 0 };
  const totalRemaining = Math.max(1, state.readRemainTenths + state.skimRemainTenths);
  let readTake = Math.round(remaining * (state.readRemainTenths / totalRemaining));
  readTake = Math.max(0, Math.min(readTake, state.readRemainTenths, remaining));
  let skimTake = Math.max(0, Math.min(remaining - readTake, state.skimRemainTenths));
  let diff = remaining - (readTake + skimTake);
  if (diff > 0) {
    const addRead = Math.min(diff, Math.max(0, state.readRemainTenths - readTake));
    readTake += addRead;
    diff -= addRead;
...
export function marginalMinutesForTenths(state: PlanningState, tenths: number): number {
...
export function consumeTenths(
  ...
  const take = Math.max(0, Math.min(state.remainingTenths, Math.round(tenths || 0)));
  ...
  return { tenths: take, readTenths: split.readTenths, skimTenths: split.skimTenths, mins };
}
...
export function totalHoursFallback(
  ...
  minutesPerPage: (difficulty: number, constraints: PlannerProjectV1['constraints']) => number,
): number {
