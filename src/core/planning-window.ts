import { DAYS_PER_WEEK, MS_PER_DAY } from './date-constants';
...
export function targetEndDateKey(
  startDateKey: string,
  ...
  const months = Math.max(
    MIN_HORIZON_MONTHS,
    horizonMonths || MIN_HORIZON_MONTHS,
  );
  const days = Math.max(
    1,
    Math.round(months * WEEKS_PER_MONTH_APPROX * DAYS_PER_WEEK),
  );
...
export function horizonMonthsFromEndDate(
  startDateKey: string,
  endDateKey: string,
): number {
  ...
  return Math.max(
    MIN_HORIZON_MONTHS,
    days / (DAYS_PER_WEEK * WEEKS_PER_MONTH_APPROX),
  );
}
