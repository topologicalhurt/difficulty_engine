...
import { targetEndDateKey, horizonMonthsFromEndDate } from './planning-window';
import { readingScopeSettingsForProject } from './reading-scope';
import type {
  AutopilotGoal, AutopilotProposal, AutopilotWizardState, ConstraintSet,
  EngineSnapshot, PlannerOptimizationInput,
...
export async function createAutopilotProposal(
  ...
  const specs = candidateSpecs(project, wizard);
  for (let index = 0; index < specs.length; index += 1) {
    const spec = specs[index];
    const candidateProject = applyCandidate(project, spec.patch);
