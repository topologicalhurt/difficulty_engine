...
export function buildOverlapMap(
  ...
  clusters.forEach((cluster) => {
    cluster.pruning.forEach((prune) => {
      ...
      current.skimFrac =
        1 - (1 - current.skimFrac) * (1 - clamp(prune.overlapFrac * prune.prereqPenalty, 0, 0.92));
...
export function buildPlanningStates(
  ...
  return [...schedulePlan.items]
    .sort((left, right) => (left.ds - right.ds) || (left.lane - right.lane) || left.short.localeCompare(right.short))
    .map((item) => {
      const overlap = overlapMap[item.id] || { skimFrac: 0, timeSaved: 0, reasons: [] };
      const totalTenths = Math.max(0, Math.round(item.pages * 10));
      const skimTenths = clamp(Math.round(totalTenths * clamp(overlap.skimFrac || 0, 0, 0.92)), 0, totalTenths);
      const readTenths = totalTenths - skimTenths;
      const strictMinPg = item.strictMinPg || pageBounds(project.constraints).minPg;
      const effectiveMinPg = item.effectiveMinPg || item.strictMinPg || strictMinPg;
      return {
        id: item.id,
        short: item.short,
        title: item.title,
        ...
        targetHrs: item.hours || totalHoursFallback(item.pages, item.scheduleDifficulty, project, minutesPerPage),
