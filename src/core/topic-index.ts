...
import type { CorpusSnapshot, InternalTopicNode, TopicCandidate, TopicIndex } from './internal-types';
...
export function buildTopicIndexForCorpus(corpus: CorpusSnapshot): TopicIndex {
  ...
  Object.values(corpus.byId).forEach((book) => {
    const ranked = Object.entries(book.phraseCounts)
      .map(([phrase, tf]) => {
        const rarity = Math.log(1 + corpus.docCount / Math.max(1, corpus.phraseDf[phrase] || 1));
        const score = tf * rarity * (1 + Math.min(1, phrase.split(' ').length * 0.18));
    ...
    ranked.forEach((topic) => {
      ...
      current.coverage.push({ bookId: book.id, weight: topic.score, chapterIdxs });
      if (chapterIdxs.length) {
        current.chapterAnchors.push(...chapterIdxs.map((idx) => ({ bookId: book.id, idx })));
      }
    });

    const weightedRarity =
      sum(ranked.map((topic) => topic.score * topic.rarity)) / Math.max(1, sum(ranked.map((topic) => topic.score)));
    const breadth = ranked.length;
    const complexity = clamp(
      TOPIC_COMPLEXITY_BASE +
        book.seedEstimate * BOOK_COMPLEXITY_SEED_WEIGHT +
        weightedRarity * BOOK_COMPLEXITY_RARITY_WEIGHT +
        clamp(breadth / BOOK_COMPLEXITY_BREADTH_DIVISOR, 0, 1) * BOOK_COMPLEXITY_BREADTH_WEIGHT +
        clamp(book.lexicalDensity * BOOK_COMPLEXITY_LEXICAL_MULTIPLIER, 0, 1) * BOOK_COMPLEXITY_LEXICAL_WEIGHT,
      1,
      10,
    );
  ...
  Object.values(topicsById).forEach((topic) => {
    ...
    const chapterSpread = unique(topic.chapterAnchors.map((anchor) => `${anchor.bookId}:${anchor.idx}`)).length;
    ...
    topic.learnedComplexity = round1(
      clamp(
        TOPIC_COMPLEXITY_BASE +
          ...
          clamp(chapterSpread / 8, 0, 1) * TOPIC_COMPLEXITY_CHAPTER_SPREAD_WEIGHT,
        1,
        10,
      ),
    );
