...
function normalizeDisplayGroups(value: unknown): Record<string, number> {
  ...
  const entries = Object.entries(value as Record<string, unknown>)
    .map(([key, rawWeight]) => [normalizeString(key), normalizeNumber(rawWeight, 1, 0, 100)] as const)
...
export function normalizeConstraints(value: unknown): ConstraintSet {
  ...
  const studyWeekdays = normalizeWeekdays(raw.studyWeekdays, defaults.studyWeekdays);
  const weekdaysCustom = normalizeBoolean(raw.weekdaysCustom) && studyWeekdays.length > 0;
  ...
  const constraints: ConstraintSet = {
    ...
    dailyBookMode: normalizeDailyBookMode(normalizeString(raw.dailyBookMode) || defaults.dailyBookMode),
    emptyDayPolicy: normalizeEmptyDayPolicy(normalizeString(raw.emptyDayPolicy) || defaults.emptyDayPolicy),
    bookOrderPolicy: normalizeBookOrderPolicy(normalizeString(raw.bookOrderPolicy) || defaults.bookOrderPolicy),
    schedAlgo: normalizeSchedAlgo(normalizeString(raw.schedAlgo) || defaults.schedAlgo),
    feasibilityMode: normalizeFeasibilityMode(normalizeString(raw.feasibilityMode) || defaults.feasibilityMode),
    backfillMode: normalizeBackfillMode(normalizeString(raw.backfillMode) || defaults.backfillMode),
    prereqMode: normalizePrereqMode(normalizeString(raw.prereqMode) || defaults.prereqMode),
    skimRatio: normalizeNumber(raw.skimRatio, defaults.skimRatio, 0, 1),
    prereqRetention: normalizeNumber(raw.prereqRetention, defaults.prereqRetention, 0, 1),
    ...
    compressCurve: normalizeCompressCurve(normalizeString(raw.compressCurve) || defaults.compressCurve),
    ...
    diffCurveFloorPoint: normalizeNumber(raw.diffCurveFloorPoint, defaults.diffCurveFloorPoint, 0, 0.45),
    diffCurveCeilingPoint: normalizeNumber(raw.diffCurveCeilingPoint, defaults.diffCurveCeilingPoint, 0.55, 1),
    diffRamp: normalizeNumber(raw.diffRamp, defaults.diffRamp, 0.1, 5),
    applyOverlapSkim: raw.applyOverlapSkim == null ? defaults.applyOverlapSkim : normalizeBoolean(raw.applyOverlapSkim),
    boostUnused: raw.boostUnused == null ? defaults.boostUnused : normalizeBoolean(raw.boostUnused),
    boostStrength: normalizeNumber(raw.boostStrength, defaults.boostStrength, 0, 2),
    mutualEnabled: raw.mutualEnabled == null ? defaults.mutualEnabled : normalizeBoolean(raw.mutualEnabled),
    mutualOversize: raw.mutualOversize === 'strict' ? 'strict' : defaults.mutualOversize,
    autoRD: raw.autoRD == null ? defaults.autoRD : normalizeBoolean(raw.autoRD),
    rdMinChain: normalizeNumber(raw.rdMinChain, defaults.rdMinChain, 1, 100, true),
    ...
    excComp: raw.excComp == null ? defaults.excComp : normalizeBoolean(raw.excComp),
  ...
  constraints.diffMapMax = Math.max(constraints.diffMapMin, constraints.diffMapMax);
...
export function normalizeUiPreferences(raw: Record<string, unknown>): PlannerProjectV1['uiPreferences'] {
  return {
    ganttView:
      raw.uiPreferences && typeof raw.uiPreferences === 'object'
        ? (String((raw.uiPreferences as Record<string, unknown>).ganttView) === 'diagnostics'
            ? 'diagnostics'
    ...
    planColorMode:
      raw.uiPreferences && typeof raw.uiPreferences === 'object'
        ? normalizePlanColorMode(
            normalizeString((raw.uiPreferences as Record<string, unknown>).planColorMode),
          )
