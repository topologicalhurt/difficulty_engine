...
export function displayGroupFromBooks(
  ...
  const groups = unique(knownGroups.map((group) => String(group || '').trim()).filter(Boolean));
  if (!groups.length) return 'Core';
  const terms = new Set(tokenizeWords([book.title, asArray(book.subjects).join(' '), book.publisher].join(' ')));
  ...
  existingBooks.forEach((other) => {
    ...
    const otherTerms = new Set(
      tokenizeWords([other.title, asArray(other.subjects).join(' '), other.publisher].join(' ')),
    );
    let shared = 0;
    terms.forEach((term) => {
      if (otherTerms.has(term)) shared += 1;
    });
    const similarity = shared / Math.max(1, terms.size + otherTerms.size - shared);
  ...
  return groups.sort((left, right) => (counts[left] || 0) - (counts[right] || 0) || left.localeCompare(right))[0] || groups[0];
}

export function extractCorpus(project: PlannerProjectV1): CorpusSnapshot {
  const books = Object.entries(project.library.books).map(([id, book]) => corpusBookFromRecord(id, book));
  ...
  const sortedPages = books.map((book) => book.pages).sort((left, right) => left - right);
