...
export const CONSTRAINT_CONTRACTS: WiringContract[] = CONSTRAINT_FIELDS.map(
  (field) => ({
    id: `constraint.${String(field.key)}`,
    surface: field.group === 'Graph Controls' ? 'graphs' : 'constraints',
    control: field.label,
    command:
      field.key === 'studyWeekdays' ? 'updateConstraints' : 'updateConstraint',
    projectReads: [`constraints.${String(field.key)}`],
    projectWrites:
      field.key === 'dpw'
        ? [
            'constraints.dpw',
            'constraints.weekdaysCustom',
            'constraints.studyWeekdays',
          ]
    ...
    notes:
      'Every planner constraint mutates the canonical project and must rebuild the engine snapshot.',
  }),
);

export function constraintContractId(
