...
function validLinkedIds(project: PlannerProjectV1, sourceId: string, ids: string[]): string[] {
  return unique(ids)
...
function cloneBook(book: BookRecord): BookRecord {
  return {
    ...
    enrichment: {
      ...
      provenance: book.enrichment.provenance ? { ...book.enrichment.provenance } : undefined,
    },
  };
}
...
export function withBookRelationPatch(
  ...
  const books = Object.fromEntries(
    Object.entries(project.library.books).map(([id, book]) => [id, cloneBook(book)]),
  );
  const source = books[sourceId];
  if (!source) return project;

  if (patch.manualPrereqs) {
    source.manualPrereqs = validLinkedIds(project, sourceId, patch.manualPrereqs);
  }

  if (patch.manualDependents) {
    const selected = new Set(validLinkedIds(project, sourceId, patch.manualDependents));
    Object.values(books).forEach((book) => {
      if (book.id === sourceId) return;
      const prereqs = new Set(book.manualPrereqs.filter((id) => id !== sourceId));
      if (selected.has(book.id)) prereqs.add(sourceId);
      book.manualPrereqs = [...prereqs].filter((id) => Boolean(books[id])).sort();
    });
  }

  if (patch.manualCoStudy) {
    const selected = new Set(validLinkedIds(project, sourceId, patch.manualCoStudy));
    source.manualCoStudy = [...selected];
    Object.values(books).forEach((book) => {
      if (book.id === sourceId) return;
      const coStudy = new Set(book.manualCoStudy.filter((id) => id !== sourceId));
      if (selected.has(book.id)) coStudy.add(sourceId);
      book.manualCoStudy = [...coStudy].filter((id) => Boolean(books[id])).sort();
    });
  }
