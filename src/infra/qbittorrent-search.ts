...
export function qbittorrentSearchPatterns(request: DocumentAcquisitionRequest): string[] {
  ...
  return Array.from(new Set([
    ...
    title,
  ].filter(Boolean))).slice(0, isbn ? PLUGIN_SEARCH_FALLBACK_LIMIT : 1);
}
...
export function shouldStopAfterSearchPattern(
  ...
  return Boolean(normalizedBookIsbn(request.book.isbn) && pattern === normalizedBookIsbn(request.book.isbn) && candidates.length);
}
...
export function candidatesFromSearchResults(
  ...
  return results.map((result, index): DocumentCandidate | null => {
    ...
    const pluginName = allowedPlugins.find((plugin) =>
      result.siteUrl ? sourceIsAllowed(result.siteUrl, [hostFromUrl(plugin.url)]) : false,
    )?.name ?? allowedPlugins[0]?.name ?? '';
    return {
      id: `${idPrefix}:${index}`,
      provider: 'qbittorrent',
      title,
      sourceUrl: result.fileUrl ?? result.descrLink ?? '',
      contentKind: contentKindFromUrl(result.fileName || result.fileUrl || ''),
      accessBasis: accessBasisForSearchResult(result, pluginName, request),
      confidence: Math.min(0.96, 0.42 + matchScore * 0.42 + Math.min(0.12, seeders / 100)),
      matchScore,
      seeders,
      ...
      sizeBytes: result.fileSize && result.fileSize > 0 ? result.fileSize : undefined,
    };
  }).filter((candidate): candidate is DocumentCandidate => Boolean(candidate?.sourceUrl));
}
