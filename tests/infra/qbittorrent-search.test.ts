...
describe('qBittorrent search precision', () => {
  ...
  it('rejects whitelisted plugin results when access basis is unknown', async () => {
    const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
      if (url.endsWith('/api/v2/auth/login')) {
        return new Response('Ok.', {
          status: 200,
      ...
      if (url.includes('/api/v2/search/results?')) {
        return Response.json({
          ...
          results: [
            {
              fileName: 'Precise Systems 9781111111111.pdf',
  ...
  it('continues past unknown-license ISBN hits to find lawful title matches', async () => {
    const searchStartBodies: URLSearchParams[] = [];
    const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
      if (url.endsWith('/api/v2/auth/login')) {
        return new Response('Ok.', {
      ...
      if (url.endsWith('/api/v2/search/start')) {
        const body = init?.body as URLSearchParams;
        searchStartBodies.push(body);
        return Response.json({ id: searchStartBodies.length });
      }
      if (url.includes('/api/v2/search/results?')) {
        const params = new URL(url).searchParams;
        ...
        return Response.json({
          ...
          results:
            id === 1
              ? [
                  {
                    fileName: 'Precise Systems 9781111111111 unknown.pdf',
                    fileUrl: 'magnet:?xt=urn:btih:unknownisbn',
                    siteUrl: 'https://archive.org/details/unknown-isbn',
                    nbSeeders: 40,
                    nbLeechers: 1,
                    fileSize: 12_000,
                  },
