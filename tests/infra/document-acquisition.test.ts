...
describe('qBittorrent document provider', () => {
  function qbitPolicy(patch: { sourceSettings?: SourceSettings } = {}): ReturnType<typeof defaultDocumentAcquisitionPolicy> {
  ...
  it('does not treat imported local torrent file paths as browser-addable torrent sources', async () => {
    ...
    const candidates = await provider.findCandidates({
      book: { ...EXAMPLE_BOOK, title: 'Fixture Book', sourcePath: '/Users/connor/private/book.torrent' },
      policy: qbitPolicy(),
    });
  ...
  it('selects the best matching file inside multi-file torrents and rejects bundled noise', async () => {
    ...
    const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      if (url.endsWith('/api/v2/torrents/info')) {
        return Response.json([{ hash: 'abc123', name: 'Linear Algebra Done Right', state: 'pausedDL', progress: 0.25 }]);
      }
      if (url.includes('/api/v2/torrents/files?')) {
        return Response.json([
          { index: 0, name: 'Preview/sample.pdf', size: 1000, progress: 1 },
          { index: 1, name: 'Linear Algebra Done Right 4th Edition.pdf', size: 10_000, progress: 0.25 },
          { index: 2, name: 'Unrelated Topology Notes.pdf', size: 8_000, progress: 1 },
        ]);
      }
      if (url.endsWith('/api/v2/torrents/filePrio')) {
        const body = init?.body as URLSearchParams;
      ...
      if (url.endsWith('/api/v2/torrents/start') || url.endsWith('/api/v2/torrents/resume')) {
        return new Response('Ok.', { status: 200 });
      }
      throw new Error(`Unexpected URL ${url}`);
    });
    ...
    const acquired = await provider.acquire(candidate, {
      book: { ...EXAMPLE_BOOK, title: 'Linear Algebra Done Right', sourcePath: null },
      policy: qbitPolicy(),
    });

    expect(acquired?.storagePath).toBe('/tmp/difficulty/Linear Algebra Done Right 4th Edition.pdf');
  ...
  it('keeps completed PDFs openable even when byte extraction is unavailable', async () => {
    const fetchImpl = vi.fn(async (url: string) => {
      ...
      if (url.endsWith('/api/v2/torrents/add')) return new Response('Ok.', { status: 200 });
      if (url.endsWith('/api/v2/torrents/info')) {
        return Response.json([{ hash: 'abc123', name: 'Practical Electronics for Inventors', state: 'stalledUP', progress: 1 }]);
      }
      if (url.includes('/api/v2/torrents/files?')) {
        return Response.json([
          { index: 0, name: 'Practical Electronics for Inventors 4th Edition.pdf', size: 10_000, progress: 1 },
        ]);
      }
      if (url.endsWith('/api/v2/torrents/filePrio')) return new Response('Ok.', { status: 200 });
      if (url.endsWith('/api/v2/torrents/start') || url.endsWith('/api/v2/torrents/resume')) {
        return new Response('Ok.', { status: 200 });
      }
    ...
    const acquired = await provider.acquire(candidate, {
      book: { ...EXAMPLE_BOOK, title: 'Practical Electronics for Inventors', sourcePath: null },
      policy: qbitPolicy(),
    });
  ...
  it('uses the bridge data root as qBittorrent save path when local settings are relative', async () => {
    ...
    const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
      if (url.endsWith('/__health')) {
        return Response.json({ ok: true, dataRoot: '/absolute/data/documents' });
      }
      if (url.endsWith('/api/v2/auth/login')) {
        return new Response('Ok.', {
          status: 200,
  ...
  it('uses whitelisted qBittorrent search plugins to produce lawful document candidates', async () => {
    ...
    expect(searchStartBodies[0]?.get('pattern')).toBe('Fixture Book Author Name');
