...
import { calendarBadges, calendarDetailText, selectPlanViewModel } from '../../src/app/selectors/plan';
...
const enrichmentProvider: EnrichmentProvider = {
  fetchBook: vi.fn(async ({ book }) => ({
    ...
    enrichment: {
      ...book.enrichment,
      chapters: book.enrichment.chapters.length ? book.enrichment.chapters : ['Start', 'Middle'],
...
function matrixBook(id: string, title: string, seed: number, pages: number, subjects: string[]) {
  return makeBook({
    id,
    title,
...
function matrixProject(): PlannerProjectV1 {
  return makeProject({
    books: {
      ...
      b: { ...matrixBook('b', 'Beta', 5, 180, ['beta', 'shared']), manualPrereqs: ['a'], manualCoStudy: ['c'] },
      c: { ...matrixBook('c', 'Gamma', 7, 260, ['gamma', 'shared']), manualPrereqs: ['a', 'b'], manualCoStudy: ['b'] },
    },
    constraints: { par: 2 },
  });
}
...
function expectSnapshotUpdate(store: PlannerStore, action: () => void): void {
  const events: string[] = [];
  const unsubscribe = store.subscriptions.subscribeEvents((event) => events.push(event.type));
  action();
  unsubscribe();
...
describe('planner parameter wiring matrix', () => {
  it('recomputes through the canonical snapshot for representative constraint controls', () => {
    const cases: Array<[keyof ConstraintSet, ConstraintSet[keyof ConstraintSet]]> = [
      ['hpd', 4],
      ['tl', 24],
      ['par', 3],
      ['minPg', 3],
    ...
    cases.forEach(([key, value]) => {
      ...
      expectSnapshotUpdate(store, () => store.commands.updateConstraint(key, value as never));
  ...
  it('classifies every user-facing constraint with explicit effect metadata', () => {
    expect(CONSTRAINT_FIELDS.every((field) => Boolean(field.effect))).toBe(true);
    expect(CONSTRAINT_FIELDS.filter((field) => field.effect === 'display_only').map((field) => field.key)).toEqual(
    ...
    expect(CONSTRAINT_FIELDS.find((field) => field.key === 'gam')?.effect).toBe('workload_time');
    expect(CONSTRAINT_FIELDS.find((field) => field.key === 'gam')?.group).toBe('Daily Workload');
  });
  ...
  it('keeps plan color mode display-only while updating Gantt and calendar colors', () => {
    ...
    expect(store.selectors.getProject().uiPreferences.planColorMode).toBe('difficulty_gradient');
    ...
    expect(selectPlanViewModel(store.selectors.getState()).colors.mode).toBe('detected_genre');
  });
  ...
  it('keeps calendar log selection UI-only while selecting the inspected book', () => {
    ...
    if (!firstEntry) throw new Error('Expected a calendar entry in the fixture plan.');
    ...
    expect(state.ui.selectedCalendarEntry).toEqual({ dateKey: firstDay, bookId: firstEntry.bookId });
    expect(viewModel.selectedCalendarEntry).toEqual({ dateKey: firstDay, bookId: firstEntry.bookId });
  ...
  it('wires Sunday and seven-day study weeks into the day-plan selector path', () => {
    const store = matrixStore();
    expectSnapshotUpdate(store, () => store.commands.updateConstraint('dpw', 7));
    expect(store.selectors.getProject().constraints.studyWeekdays).toEqual([0, 1, 2, 3, 4, 5, 6]);
    ...
    expect(viewModel.calendarWeeks.flatMap((week) => week.days).some((day) => day.dayLabel === 'Sun')).toBe(true);
  });

  it('projects graph settings through the graph selector instead of stale view state', () => {
    const store = matrixStore();
    store.commands.updateConstraint('tr', true);
    const reducedEdges = selectGraphRenderModel(store.selectors.getState()).prerequisiteEdges.length;

    store.commands.updateConstraint('tr', false);
    const fullEdges = selectGraphRenderModel(store.selectors.getState()).prerequisiteEdges.length;

    store.commands.updateConstraint('mutualEnabled', false);
    const disabledCoStudy = selectGraphRenderModel(store.selectors.getState()).coStudyGroups.length;
  ...
  it('wires calendar actuals and deferrals into the same plan projection used by the UI', () => {
    const store = matrixStore();
    const firstDay = Object.keys(store.selectors.getSnapshot().dayPlan.byDate).sort()[0];
    const firstEntry = store.selectors.getSnapshot().dayPlan.byDate[firstDay]?.[0];
    ...
    let entry = selectPlanViewModel(store.selectors.getState())
      ...
      .find((item) => item.bookId === firstEntry.bookId);
	    ...
	    expect(Math.round(((entry?.readPages ?? 0) + (entry?.skimPages ?? 0)) * 10) / 10).toBe(6.5);

    expectSnapshotUpdate(store, () => store.commands.deferCalendarEntry(firstDay, firstEntry.bookId));
  ...
  it('keeps floor details out of default calendar badges but available in detail text', () => {
    ...
    expect(calendarBadges(entry ?? Object.values(store.selectors.getSnapshot().dayPlan.byDate).flat()[0]).some((item) => item.label.includes('Floor'))).toBe(false);
    expect(calendarDetailText(entry ?? Object.values(store.selectors.getSnapshot().dayPlan.byDate).flat()[0])).toContain('Floor relaxed');
  });
  ...
  it('exposes boost days through compact calendar details', () => {
    ...
    expect(calendarDetailText(boosted ?? Object.values(store.selectors.getSnapshot().dayPlan.byDate).flat()[0])).toContain('Boost day');
  });
  ...
  it('keeps search, enrichment, and book metadata wired into library and snapshot outputs', async () => {
    ...
    expect(Object.values(project.library.books).some((item) => item.title === 'Matrix Search Book')).toBe(true);
    expect(project.library.books[added?.id ?? '']?.subjects).toContain('enriched');
    expect(store.selectors.getSnapshot().schedulePlan.byId[added?.id ?? '']).toBeTruthy();
  });
});
