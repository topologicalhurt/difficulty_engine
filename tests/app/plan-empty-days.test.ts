...
import type { BookRecord, EngineSnapshot, EnrichmentProvider, Logger, PlannerEngine, PlannerProjectV1 } from '../../src/core/types';
...
const enrichmentProvider: EnrichmentProvider = {
  ...
  searchBooks: async () => ({ results: [], hasMore: false, nextOffset: 0, mode: 'search' }),
};
...
function contiguousMissingDates(snapshot: EngineSnapshot): string[] {
  ...
  for (let index = 1; index < dates.length; index += 1) {
    ...
    for (prev.setUTCDate(prev.getUTCDate() + 1); prev < current; prev.setUTCDate(prev.getUTCDate() + 1)) {
...
function parallelConflictFixture(): PlannerProjectV1 {
  return parseProject(JSON.stringify({
    ...
    library: {
      books: {
        a: book('a', { title: 'Alpha Systems', pages: 80, manualSeedDifficulty: 8, lockDiff: true }),
        b: book('b', { title: 'Beta Systems', pages: 80, manualSeedDifficulty: 8, lockDiff: true }),
        c: book('c', { title: 'Gamma Systems', pages: 80, manualSeedDifficulty: 8, lockDiff: true }),
        d: book('d', { title: 'Delta Systems', pages: 80, manualSeedDifficulty: 8, lockDiff: true }),
      },
    },
...
describe('plan empty-day and parallel-fill projection', () => {
  it('labels fixture calendar padding as outside the plan instead of no-study days', () => {
    ...
    const emptyCells = selectPlanViewModel(store.selectors.getState())
      ...
      .filter((day) => day.entries.length === 0 && day.missedEntries.length === 0);

    expect(emptyCells.length).toBeGreaterThan(0);
    expect(emptyCells.every((day) => day.status === 'outside_plan')).toBe(true);
    expect(new Set(emptyCells.map((day) => day.statusLabel))).toEqual(new Set(['Before plan', 'After finish']));
  });
  ...
  it('reports strict floor math when parallel slots cannot all fit', () => {
    ...
    expect(snapshot.renderModel.warnings.some((warning) => warning.code === 'strict-parallel-floor-conflict')).toBe(true);
    expect(snapshot.renderModel.warnings.map((warning) => warning.message).join('\n')).not.toContain('only 7 book(s) were startable');
  });
  ...
  it('records explained empty study days when preserving release gaps', () => {
    const project: PlannerProjectV1 = {
      ...
      manualOverrides: { schedule: { delayed: { ds: 4 } }, deferred: {}, actuals: {} },
      constraints: {
        ...DEFAULT_CONSTRAINTS,
        sd: '2026-01-05',
        dpw: 7,
    ...
    expect(snapshot.dayPlan.startability.emptyStudyDays.every((day) => day.reason === 'waiting_for_release')).toBe(true);
  });
});
