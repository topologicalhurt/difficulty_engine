...
import { selectConstraintsViewModel, selectGraphOptionsViewModel } from '../../src/app/selectors/constraints';
...
describe('constraints view model', () => {
  it('separates primary and advanced fields by group disclosure state', () => {
    ...
    let viewModel = selectConstraintsViewModel(plannerStore.selectors.getState());
    const difficultyGroup = viewModel.groups.find((group) => group.group === 'Difficulty Mapping');
    const planWindowGroup = viewModel.groups.find((group) => group.group === 'Plan Window');
    ...
    expect(difficultyGroup?.fields.map((field) => field.key)).toEqual(['diffMapMode']);
    ...
    expect(
      viewModel.groups.find((group) => group.group === 'Difficulty Mapping')?.fields
        .map((field) => field.key),
    ).toContain('diffRamp');
  });
  ...
  it('returns focused field and selected option explanations', () => {
    ...
    const viewModel = selectConstraintsViewModel(plannerStore.selectors.getState());

    expect(viewModel.explanation.title).toBe('Display map mode');
    expect(viewModel.explanation.selectedOptionDetail).toContain('current library');
  });

  it('builds a difficulty mapping chart model from current books', () => {
    const viewModel = selectConstraintsViewModel(constraintsStore().selectors.getState());

    expect(viewModel.difficultyMapping.curve.length).toBeGreaterThan(10);
    expect(viewModel.difficultyMapping.identity).toHaveLength(viewModel.difficultyMapping.curve.length);
    expect(viewModel.difficultyMapping.legendLabels).toEqual(['Identity', 'Current curve', 'Books']);
    expect(viewModel.difficultyMapping.books).toHaveLength(2);
    expect(viewModel.difficultyMapping.books.every((book) => book.color.startsWith('hsl('))).toBe(true);
    expect(viewModel.difficultyMapping.books.some((book) => book.plotOffset !== 0)).toBe(true);
  ...
  it('exposes paired compression curve options and clip guide data', () => {
    ...
    expect(viewModel.difficultyMapping.floorGuide?.rawDifficulty).toBeGreaterThan(1);
    expect(viewModel.difficultyMapping.ceilingGuide?.rawDifficulty).toBeLessThan(10);
  });

  it('keeps Graphs tab options limited to graph display behavior', () => {
    const viewModel = selectGraphOptionsViewModel(constraintsStore().selectors.getState());
    const keys = viewModel.groups.flatMap((group) => group.fields.map((field) => field.key));
