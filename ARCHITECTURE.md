...
- Repeated primitive operations should use canonical helpers: string compaction/deduplication in `src/core/utils.ts`, external-source matching in `src/core/matchers.ts`, display formatting in `src/ui/format.ts`, and document content ranking in `src/infra/qbittorrent-file-kinds.ts`.
- No inline event handlers, runtime monkey-patching, or retired adapter branches remain in the production path.
- Source audits enforce file-size caps, default-export bans, and deprecated-runtime removal.
